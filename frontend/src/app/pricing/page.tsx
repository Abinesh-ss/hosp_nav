...
export default function PricingPage() {
  return (
    <section className="py-20 page-transition">
      <div className="content-wrap">
        ...
        <div className="grid gap-8 md:grid-cols-3 max-w-6xl mx-auto">
          {pricingPlans.map(p => (
            <div key={p.name} className={`flex flex-col rounded-xl border p-8 transition-all duration-300 ${p.isPrimary ? 'border-primary bg-gray-900 text-white shadow-2xl scale-[1.02]' : 'bg-card border-gray-200'}`}>
              <h3 className="text-3xl font-bold mb-6">{p.name}</h3>
              <p className="mb-8">
                <span className={`text-5xl md:text-6xl font-extrabold ${p.isPrimary ? 'text-white' : 'text-foreground'}`}>{p.price}</span>
                <span className={`text-lg font-medium ml-3 ${p.isPrimary ? 'text-gray-400' : 'text-muted-foreground'}`}>{p.unit}</span>
              </p>

              <Link href="/login" className={`inline-flex items-center justify-center rounded-md text-sm font-medium h-10 px-4 py-2 transition-colors w-full border ${p.isPrimary ? 'bg-primary text-primary-foreground hover:bg-indigo-600 border-primary/50' : 'bg-white text-primary hover:bg-gray-50 border-primary'}`}>
